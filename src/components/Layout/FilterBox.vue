...
<script>
import { addParamsToLocation } from "@/utils/Urls";
  ...
  watch: {
    currentSort: function(newSort, oldSort) {
      ...
      const posts = this.posts.slice(0).sort((a, b) => {
        ...
        if (a[sortItem.sortProp] === b[sortItem.sortProp]) {
      ...
      this.updateSort(sortItem);
    }
  },
  methods: {
    updateSort(sortItem) {
      const urlParams = this.$router.history.current.query;
      addParamsToLocation({
        ...urlParams,
        sortProp: sortItem.sortProp,
